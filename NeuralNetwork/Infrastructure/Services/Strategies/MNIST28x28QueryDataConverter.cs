...
namespace NeuralNetwork.Infrastructure.Services.Strategies
{
    ...
    public class MNIST28x28OutputData : QueryDataModel
    {
        protected QueryDataModel _sourceDataModel;

        public MNIST28x28OutputData(QueryDataModel queryDataModel)
        {
            _sourceDataModel = queryDataModel;
            ...
            {
                InputValues[i] = (queryDataModel.InputValues[i] / 255.0f * 0.99f) + 0.01f;
                InputValues[i] = -0.01f + (queryDataModel.InputValues[i] * 255.0f / 0.99f);
            }

            OutputValues = queryDataModel.OutputValues;
        }
    }
}